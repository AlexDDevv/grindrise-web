 ...
 * une signature HMAC en base64url : il approche les 180 caractères. Sans ce
 * relèvement, AUCUN lien de livraison ne fonctionne.
 */
...
export async function downloadRoutes(
  ...
  app.get<{ Params: { token: string } }>('/api/download/:token', async (request, reply) => {
    ...
    if (!commande) {
      ...
      logger.error('Token valide sans commande correspondante', { sessionId: orderId });
      return reply.code(403).send({ error: 'Lien invalide ou expiré.' });
    }
    ...
    if (!existsSync(chemin)) {
      logger.error('PDF introuvable dans le volume', { sessionId: orderId, chemin });
      return reply.code(500).send({ error: 'Fichier indisponible.' });
    }
