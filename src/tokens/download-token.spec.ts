...
describe('tokens de téléchargement', () => {
  it("vérifie un token qu'il vient de signer", () => {
    ...
    expect(verifyDownloadToken(token, SECRET)).toBe('ord_1');
  });
  ...
  it('rejette un token expiré', () => {
    ...
    const token = signDownloadToken('ord_1', SECRET, 7, emis);
  ...
  it('accepte un token encore dans sa fenêtre de validité', () => {
    ...
    expect(verifyDownloadToken(token, SECRET, new Date('2026-01-05T00:00:00Z'))).toBe('ord_1');
  });
